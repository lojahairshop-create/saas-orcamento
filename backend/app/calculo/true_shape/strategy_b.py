...
class StrategyBNfp:
    ...
    def execute(self, parts: List[CanonicalPartGeometry], rotations: List[float] = [0.0], timeout_budget: float = None) -> List[Placement]:
        placements = []
        placed_polys = []
        ...
        for part in parts:
            if timeout_budget and (time.time() - start_global) > timeout_budget:
                print("DEBUG: Soft budget timeout reached!")
                break
